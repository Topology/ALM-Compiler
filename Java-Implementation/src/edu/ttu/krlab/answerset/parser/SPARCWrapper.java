...
public class SPARCWrapper {
	...
	public static ArrayList<AnswerSet> solve (String sparcProgram, String pathToSPARCJar, String pathToJava, String PathToASPSolver, String whichSolver) {

		String answerSetsString = computeAnswerSets(sparcProgram, pathToSPARCJar, pathToJava, PathToASPSolver, whichSolver);
	...
	private static String computeAnswerSets(String sparcProgram, String pathToSPARCJar, String pathToJava, String pathToASPSolver, String whichSolver) {
		...
		if (OsUtils.errors.toString().length() > 31 && OsUtils.errors.toString().substring(0,31).equals("Error: Unable to access jarfile")) {
			throw new RuntimeException("Unable to find Sparc Solver at given path");
		}
		if (OsUtils.errors.toString().length() > 38 && OsUtils.errors.toString().substring(0,38).equals("java.lang.UnsupportedClassVersionError")) {
			throw new RuntimeException("Newer version of java required");
		}
		if (OsUtils.errors.toString().length() > 45 && OsUtils.errors.toString().substring(32,45).equals("dlv not found")) {
			throw new RuntimeException("Unable to locate dlv solver");
		}
		if (OsUtils.errors.toString().length() > 45 && OsUtils.errors.toString().substring(32,48).equals("Clingo not found")) {
...
class OsUtils { // from Evgenii Balai and modified to meet ALM's Needs
	...
	public static void runCommand(List<String> command, String input, String aspsolverpath) {
		...
		try {
			...
			throw new RuntimeException("IOException occurred from function runCommand in class OsUtils (by the end of the function)");
