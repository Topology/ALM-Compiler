...
public class ALMCompiler {
    ...
    /**
     * Commandline Arguments: 0) filename of ALM system description
     * 
     * @param args
     */
    ...
    private static void PreModelCreatePredicatesSection(SPARCProgram pm, SymbolTable st) {
        ...
        for (FunctionEntry f : st.getFunctions()) {
            ...
            try {
                ...
                e.printStackTrace();
                IMPLEMENTATION_FAILURE("PreModelCreatePredicatesSection",
    ...
    public static final void Translate(Reader r, ALMCompilerSettings s, SymbolTable st, ErrorReport er,
        ...
        try {
            ...
            e.printStackTrace();
            IMPLEMENTATION_FAILURE("Preparing ANTLR Parser Input",
                    "Could not create ANTLRInputStream due to IOException.");
        }
    ...
    private static void TranslateRule(ASPfRule ar, SymbolTable st, SPARCProgram pm, String section) {
        ...
        if (ahead != null) {
            ...
            case ALMTerm.FUN:
                ...
                f = st.getFunctionEntry(thead, thead.getArgs().size(), st, null);
                if (f != null) {
                    if (!f.isBoolean())
                        IMPLEMENTATION_FAILURE("Translate Rule", "Non Boolean Function [" + f.getFunctionName()
                                + "] must be in a Term Relation at head of rule");
                    ...
                    if (f.isStatic())
                        ...
                        IMPLEMENTATION_FAILURE("Translate Rule",
            ...
            case ALMTerm.TERM_RELATION:
                ...
                if (thead.getName() != ALM.SYMBOL_EQ && thead.getName() != ALM.SYMBOL_NEQ)
                    IMPLEMENTATION_FAILURE("Translate Rule", "Term relation at head of rule must be = or !=");
                ...
                FunctionEntry ft;
                //translating only happens without any errors, no need for an ErrorReport here. 
                ft = st.getFunctionEntry(left, left.getArgs().size(), st, null);
                if (ft != null) {
                    if (ft.isBoolean())
                        IMPLEMENTATION_FAILURE("Translate Rule",
        ...
        if (abody != null && abody.size() > 0) {
            ...
            for (ASPfLiteral alit : abody) {
                ...
                case ALMTerm.FUN:
                    FunctionEntry f;
                    //translating only happens without any errors, no need for an ErrorReport here. 
                    f = st.getFunctionEntry(tlit, tlit.getArgs().size(), st, null);
                    if (f != null) {
                        if (!f.isBoolean())
                            IMPLEMENTATION_FAILURE("Translate Rule", "Non Boolean Function [" + tlit.toString()
                                    + "] must be in a Term Relation in body of rule");
                        // Should be caught by semantic error.
                        if (f.isStatic())
                            body.add(tlit);
                        else if (f.isFluent()) {
    ...
    private static ALMTerm TranslateTerm(ALMTerm term, List<SPARCLiteral> body, SymbolTable st, TypeChecker tc,
        ...
        case ALMTerm.FUN:
            FunctionEntry f;
            f = st.getFunctionEntry(term, term.getArgs().size(), st, null);
            if (f != null) {
                ...
                if (f.isBoolean())
                    IMPLEMENTATION_FAILURE("Translate Term",
                ...
                IMPLEMENTATION_FAILURE("Translate Rule", "Term  [" + term.toString() + "] Is Not In The Symbol Table");
                // Should be caught by semantic error.
            }
        default:
