...
public abstract class ALM {
	...
	private static ALMTerm ParseExpression(ExpressionContext expression) {
		...
		if("+".compareTo(op.getText()) == 0)
			expr = new ALMTerm(op.getText(), ALMTerm.MATH_ADD, expression);
		else if("-".compareTo(op.getText()) == 0)
	...
	private static ALMTerm ParseArithmetic(Arithmetic_termContext aterm) {
		if(aterm.arithmetic_term() == null){
			...
			if(factors.size() == 1)
				...
				ALMTerm exponent = new ALMTerm("^", ALMTerm.MATH_EXP, aterm);
			...
			else if("mod".compareTo(op.getText()) == 0)
	...
	private static ALMTerm ParseFactor(FactorContext factor) {
		...
		if("-".compareTo(first.getText()) == 0){
			...
			return neg;
		} else if("(".compareTo(first.getText()) == 0) {
	...
	private static ALMTerm ParseNegFunDef(Neg_fun_defContext nfd) {
		...
		return new ALMTerm( "!=", ALMTerm.TERM_RELATION, nfd).addArg(left).addArg(right);
	}
	...
	public static ALMTerm ParsePosFunDef(Pos_fun_defContext pfd) {
		if(pfd.term() != null){
			...
			return new ALMTerm( "=", ALMTerm.TERM_RELATION, pfd).addArg(left).addArg(right);
		}
		else {
	...
	public static ALMTerm ParseAttrDef(One_attribute_defContext attr_def) {
		return new ALMTerm("=", ALMTerm.ATTR_DEF, attr_def).addArg(ALM.ParseFunction(attr_def.function_term())).addArg(ALM.ParseTerm(attr_def.term()));
	}
